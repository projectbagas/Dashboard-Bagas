...
if menu == "Overview":
    ...
    with col2:
        ...
        ax2.bar(bar_df["Sentimen"], bar_df["Jumlah"])
    ...
    **Tidak Puas**. Visualisasi pie chart menunjukkan proporsi masing-masing
    sentimen, sedangkan bar chart menampilkan jumlah ulasan per kategori.
    """)
...
# =====================================================
    ...
    sentiment_option = st.selectbox("Pilih Sentimen:", df["sentimen"].unique())
    text_data = " ".join(df[df["sentimen"]==sentiment_option]["content"].astype(str))
...
# =====================================================
    ...
    filter_sentiment = st.selectbox("Filter Sentimen:", ["Semua"] + list(df["sentimen"].unique()))
    df_show = df if filter_sentiment=="Semua" else df[df["sentimen"]==filter_sentiment]
    st.dataframe(df_show[["content","score","sentimen"]], use_container_width=True)
...
# =====================================================
    ...
    st.markdown("Fitur ini digunakan untuk mengklasifikasikan ulasan baru ke kategori **Puas, Netral, atau Tidak Puas**.")

    model_choice = st.selectbox("Pilih Model:", ["XGBoost", "Random Forest"])
    user_review = st.text_area("Masukkan Teks Ulasan:", placeholder="Contoh: Driver ramah dan aplikasi mudah digunakan")
    ...
    if st.button("🔍 Klasifikasikan"):
        if user_review.strip()=="":
            ...
            prediction = model_xgb.predict(review_tfidf)[0] if model_choice=="XGBoost" else model_rf.predict(review_tfidf)[0]
